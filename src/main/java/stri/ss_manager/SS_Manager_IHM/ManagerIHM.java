...
public class ManagerIHM extends java.awt.Frame {
    ...
    public void blinkTrapBouton(SNMPMessage trap){
        ...
        // version 4: relayage de TRAP au top level manager TOP
        SNMPMessage trapForTopLevelManager;
        // ANALYSE DE LA CRITICITE DU TRAP pour voir le relayage
        VarBind vb = trap.getPayload().getVarBindingsList().get(1);
        // si la TRAP s'agit d'un message RMI, on relai le trap
        if(vb != null && new String(vb.getObjectValue()).matches("RMI")){
            trapForTopLevelManager = new SNMPMessage(null, this.topLevelManager, trap.getPort(), trap.getVersion(), "public".getBytes(), (byte) trap.getPduType(), trap.getPayload());
            //envoi du trap
            snmpHandler.sendtraopToTopLevelManager(trapForTopLevelManager);
        }else{
            //aucun envoi
        }
        //       
 ...
 interroger les mibs chargées.
    ...
    /**
     * Cette fonction sert à envoyer un TRAP lorsqu'on se connecte
     * en RMI à un Manager.
     * 
     * @param ipOfRemoteManagerInterface Adresse IP en chaine de caractère du Manager distant
     */
    private void sendRmiTrap(String ipOfRemoteManagerInterface){
       ...
       try{ 
        ...
        varBind.add(new VarBind(new OID("1.3.6.1.2.1.1.7"), "RMI_CONNECTION".getBytes()));
 //       varBind.add(new VarBind(new OID("1.3.6.1.6.3.1.1.5.5.0"), this.rmi_remote_manager_ip.toString().getBytes()));

        SNMPMessagePayload trapPayload = new SNMPMessagePayload(0xA7, 0, 0, varBind);
        SNMPMessage trap_message_v2 = new SNMPMessage(null, 
                                                    InetAddress.getByName(this.rmi_remote_manager_ip.getText()), 
                                                    162, 
                                                    1,
                                                    this.ManagerCommunauty.getBytes(), 
                                                    (byte) 0xA7, 
                                                    trapPayload);
        // envoi du message
        this.snmpHandler.sendTrap(trap_message_v2);
       }catch(Exception e){
    ...
    private void initComponents() {
        ...
        rmi_shutdownRemoteBouton = new javax.swing.JButton();
        ...
        rmi_oid_TextField.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                rmi_oid_TextFieldMouseClicked(evt);
            }
        });
        ...
        rmi_value_TextField.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                rmi_value_TextFieldMouseClicked(evt);
            }
        });
        ...
        rmi_shutdownRemoteBouton.setBackground(new java.awt.Color(255, 0, 0));
        rmi_shutdownRemoteBouton.setFont(new java.awt.Font("Tahoma", 1, 11)); // NOI18N
        rmi_shutdownRemoteBouton.setText("ShutDown");
        rmi_shutdownRemoteBouton.setEnabled(false);
        rmi_shutdownRemoteBouton.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                rmi_shutdownRemoteBoutonActionPerformed(evt);
            }
        });
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel1Layout.createSequentialGroup()
                .addContainerGap(javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                .addComponent(rmi_shutdownRemoteBouton)
                .addContainerGap())
        );
        ...
        ManagerPanel.setSelectedIndex(2);
   ...
   // SET OK
    ...
    private void rmi_connect_boutonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_rmi_connect_boutonActionPerformed
        ...
        if(this.rmi_connect_bouton.getText().matches("CONNECT")){
            try{
                ...
                this.rmi_shutdownRemoteBouton.setEnabled(true);
                //
                this.rmi_console.setText("RMI_CONSOLE >");
                
                try {
                    // connexion établit, on change l'ip du ManagerTopLevel par celui qui vient de se connecter
                    
                    this.snmpRemoteManagerInterface.setIpTopLevelManager(InetAddress.getLocalHost());
                } catch (UnknownHostException ex) {
                    Logger.getLogger(ManagerIHM.class.getName()).log(Level.SEVERE, null, ex);
                }
                ...
                this.rmi_shutdownRemoteBouton.setEnabled(false);
                //
                this.rmi_console.setText("RMI_CONSOLE >...");
            }
            // on ajoute cet agent dans le jTree
            
            // envoi d'un trap au manager connecté
            
            //
            
            
            this.sendRmiTrap(this.rmi_remote_manager_ip.getText());
                ...
                // déréférencement
                this.snmpRemoteManagerInterface = null;
                //
        }
    ...
    private void GetBoutonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_GetBoutonActionPerformed
        ...
        try{
            ...
            SNMPMessage req_msg         = new SNMPMessage(null, Receiver, 161, 2, this.CommunauteField.getText().getBytes(), (byte) 0xA0, payload);
            // envoi de la requête
            SNMPMessage res_msg         = snmpHandler.sendGetRequest(req_msg);
            // affichage du résultat
            this.LogTextArea.setText(res_msg.toStringforIHM());
            //
            setOidandValueOnIhmByVarBin(res_msg.getPayload().getVarBindingsList().get(0));
    ...
    private void GetNextBoutonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_GetNextBoutonActionPerformed
        ...
        try{
            ...
            //
            setOidandValueOnIhmByVarBin(res_msg.getPayload().getVarBindingsList().get(0));
            this.SetOIDField.setText(res_msg.getPayload().getVarBindingsList().get(0).getObjectId().getObjectIdStringFormat());
            this.ValueField.setText(new String(res_msg.getPayload().getVarBindingsList().get(0).getObjectValue()));
        }catch(Exception e){
            this.GetNextBouton.setEnabled(true);
            System.err.println("[IHM_ERROR.GetBouton]: " +e.getMessage());
        }
    ...
    private void rmi_oid_TextFieldMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_rmi_oid_TextFieldMouseClicked
        // TODO add your handling code here:
        this.rmi_oid_TextField.selectAll();
    }//GEN-LAST:event_rmi_oid_TextFieldMouseClicked

    private void rmi_value_TextFieldMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_rmi_value_TextFieldMouseClicked
        // TODO add your handling code here:
        this.rmi_value_TextField.selectAll();
    }//GEN-LAST:event_rmi_value_TextFieldMouseClicked

    private void rmi_shutdownRemoteBoutonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_rmi_shutdownRemoteBoutonActionPerformed
        // TODO add your handling code here:
        // éteint le Manager distant
        try{
            this.snmpRemoteManagerInterface.shutDown(); 
        }catch(Exception e){
            System.err.println("[IHM_ERROR.ShutdownBouton]: " +e.getMessage());
        }
       
    }//GEN-LAST:event_rmi_shutdownRemoteBoutonActionPerformed
    ...
    private javax.swing.JButton rmi_shutdownRemoteBouton;
